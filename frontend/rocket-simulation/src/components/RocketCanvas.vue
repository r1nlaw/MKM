  ...
  export default {
    ...
    methods: {
      ...
      startRocketUpdates() {
        ...
        }, 16); // Каждые 16 мс (60 FPS), можно уменьшить до 1 мс, если нужно
      },
      ...
      // Увеличение тяги на 5%
      increaseThrust() {
        this.thrust *= 1.05;  // Увеличиваем тягу на 5%
        this.updateRocketThrust();
      },
  
      // Уменьшение тяги на 5%
      decreaseThrust() {
        if (this.thrust > 0) {
          this.thrust *= 0.95; // Уменьшаем тягу на 5%
        }
        this.updateRocketThrust();
      },
